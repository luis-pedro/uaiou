...
void main() {
  group('publicar pedido — RF-A10.1/RF-A10.2/RNF-A10.1', () {
    test(
      'corpo de POST /orders leva lat/lng do mapa e Idempotency-Key',
      () async {
...
'''),
    ...
    test(
      'sem ponto marcado no mapa não publica (RF-A10.3) — nenhum POST sai',
      () async {
        final servidor = _Servidor();
...
'''),
    ...
    test(
      '422 INSUFFICIENT_CREDITS vira mensagem de negócio sem oferta de compra',
      () async {
        final servidor = _Servidor();
        servidor.respostas['/orders'] = [
          _resp(422, '''
...
'''),
  ...
  group('decisão de contraoferta — RF-A10.6', () {
    test(
      'aceitar chama PUT /counteroffers/{id}/decision com outcome accepted',
      () async {
        final servidor = _Servidor();
